...
`scripts/add-doc-pr` is a terminal helper for preparing docs-only pull requests with explicit safety checks.
...
By default, changes under `scripts/`, `tests/`, `.github/workflows/`, `VERSION`, `LICENSE`, and `.repo-automation.conf` are blocked.
...
- passing checks (`git diff --check` and `scripts/run-tests`)
...
Usage examples:
    ...
    scripts/add-doc-pr --plan --json
    scripts/add-doc-pr --dry-run --branch docs/my-doc-update
    scripts/add-doc-pr --plan --allow templates/
    scripts/add-doc-pr --create-pr --branch docs/my-doc-update --commit-message "docs: update contract wording" --title "Docs: update contract wording" --body-file /path/to/body.md
