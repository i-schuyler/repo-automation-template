...
`scripts/repo-automation-report-upstream` prepares upstream automation bug/feature reports from terminal with preview and redaction safeguards.
...
Usage examples:
    ...
    scripts/repo-automation-report-upstream --type bug --title "Branch cleanup classification issue" --command "scripts/branch-cleanup --plan" --expected "classifies merged branch as candidate" --actual "branch marked ambiguous" --dry-run
    scripts/repo-automation-report-upstream --type feature --title "Add provider compatibility matrix docs" --use-case "cross-provider setup churn" --proposed "new docs section and validation notes" --why-upstream "shared behavior across downstream installs" --dry-run --json
    scripts/repo-automation-report-upstream --type bug --title "Preflight divergence check mismatch" --command "scripts/codex-slice-preflight --branch feature/x" --expected "clean divergence summary" --actual "unexpected stop with stale ref" --logs-file ./redacted.log --submit
