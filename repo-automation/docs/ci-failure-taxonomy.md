...
| `fail: run-tests` | local test failure | inspect the failing test slice or uploaded `run-tests.log` |
| `fail: shellcheck` | shell lint failure | inspect the reported shell line or uploaded `shellcheck.log` |
...
- CI uploads `run-tests.log` and `shellcheck.log` when those checks fail.
