...
After Codex completes, `slice-handoff --submit` also captures `repo-automation/bin/codex-status --recent=1` JSON into run-dir artifacts (`codex-status-recent.json`, `codex-status-recent.stderr`) and renders a `CODEX RUN CONTEXT` block in explain mode. The context block includes `work_time=` when the status source provides it, and the INFO stream repeats the same summary after `codex-run`. In submit success output, the visible order is `FINAL SUMMARY`, `CODEX RUN CONTEXT`, then `PR REVIEW REQUEST`; no unlabeled trailing machine block follows the review request, and the review block includes compact Review metadata. If Codex final output is a blocker, explain mode prints `CODEX RUN CONTEXT` before `CODEX FINAL OUTPUT`.
