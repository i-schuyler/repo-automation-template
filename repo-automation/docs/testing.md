...
`RUN_TESTS_DF_BIN` or `REPO_AUTOMATION_DF_BIN` can point the low-disk check at a deterministic `df` seam for tests. `repo-automation/bin/run-tests` also reads optional defaults from `.repo-automation.conf` or `.repo-automation.local.conf`, with environment variables taking precedence. The guard stops before heavy checks when `/` drops below 1.5G free or the legacy under-15%-free threshold is crossed, and `--disk-diagnostic` prints the current snapshot plus compact top temp/cache dirs.
