...
| `slice-handoff-blocker-semantics-discoverability` | 5 | active | 2026-06-03 | slice-handoff treated codex-run success and artifact presence as sufficient, so a blocker final output could cross the PR-body-validation / submit trust boundary and open a PR. | inspect Codex final output before submit and stop on a first non-empty `blocker` line; prefer smaller trust-boundary tests, clearer contract/output-contract summaries, negative downstream-invocation assertions, and more self-describing quiet artifacts |
