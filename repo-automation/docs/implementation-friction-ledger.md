...
| `run-tests-umbrella-failure-reason-fallback` | 2 | mitigated-monitoring | 2026-06-07 | `run-tests` umbrella failures needed a fallback that could turn generic child `failed` lines into the captured child stderr/excerpt reason without sending operators to a cleaned log. | keep umbrella failure renderers able to mine the child excerpt when the first recorded failure line is generic |
...
| `codex-session-resume-docs-slice` | 0 | historical | 2026-06-02 | ids=none / score_delta=0 / signal=public-safe Codex interactive-resume-session metadata notes and future helper direction documented without implementation | none |
| `codex-status-spec` | 0 | historical | 2026-06-02 | ids=none / score_delta=0 / signal=public-safe one-helper codex-status spec drafted with JSON-first contract, warning thresholds, and integration/test plan; no implementation | none |
| `codex-status-recent-fixture-isolation` | 1 | active | 2026-06-07 | recent-session coverage needed a separate CODEX_HOME fixture tree so newest malformed files did not change single-session latest ordering, and the shellcheck pass wanted helper args referenced explicitly. | keep recent-mode smoke fixtures isolated from single-session fixtures and keep helper-arg wiring explicit |
...
| `slice-handoff-smoke-fixture-coverage` | 4 | mitigated-monitoring | PR #189 / 2026-06-01 | helper metadata referenced slice-handoff, and explain-mode coverage later needed the current helper copy synced into the smoke/execution temp repos without dirtying the worktrees. | keep metadata-driven helper-bin fixture copying in place, and skip-worktree synced temp helpers when contract coverage needs the workspace copy |
...
| `slice-handoff-contract-opacity` | 4 | mitigated-monitoring | PR #229 | the slice-handoff smoke wrapper still needs careful first-failure capture when child-helper wiring or manifest-path propagation shifts the visible failure boundary. | keep first-failure extraction, durable failure logs, and manifest-path assertions aligned |
| `slice-handoff-preflight-disk-stub` | 1 | active | PR #235 | execution-mode slice-handoff smoke could fail its preflight disk guard in low-space developer environments before the actual contract path ran. | keep execution-mode smoke using a healthy df stub so preflight contract coverage stays about slice-handoff behavior, not host disk pressure |
...
| `slice-handoff-blocker-semantics-discoverability` | 3 | mitigated-monitoring | 2026-06-05 | slice-handoff treated codex-run success and artifact presence as sufficient, so a blocker final output could cross the PR-body-validation / submit trust boundary and open a PR. | inspect Codex final output before submit and stop on a first non-empty `blocker` line; prefer smaller trust-boundary tests, clearer contract/output-contract summaries, negative downstream-invocation assertions, and more self-describing quiet artifacts |
...
| `self-modifying-helper-guard` | 4 | mitigated-monitoring | PR #210 | slice-handoff let its own helper become an edit target during execution, breaking the running script mid-run. | reject prompts that target `repo-automation/bin/slice-handoff` before run-dir creation or preflight; use direct Codex or same-branch repair for helper changes |
...
| `slice-validator-contract-test-fragility` | 2 | mitigated-monitoring | PR #227 | `py_compile` created a repo-tree `__pycache__` that tripped managed-file-check, and the first contract-test pass needed literal manifest-marker grep fixes. | keep generated Python cache out of the repo tree and prefer literal grep assertions for manifest keys |
...
- 2026-06-07 / PR #233 follow-up / slice-handoff execution-submit state drift / ids=slice-handoff-contract-opacity / score_delta=0 / signal=named slice-handoff scenario checks, per-scenario execution-submit fake-artifact isolation, repo-root preset resolution, and sourced-library ShellCheck cleanup were needed so blocker checks could trust the run-dir artifacts instead of shared submit fixtures
...
- 2026-06-05 / PR #227 / slice-validator-contract-test / ids=slice-validator-contract-test-fragility / score_delta=+2 / signal=validator contract smoke needed pycache cleanup and literal grep fixes before passing
- 2026-06-04 / PR #222 / smoke-named-check-meta / ids=slice-handoff-contract-opacity / score_delta=0 / signal=child named-check failures now preserve internal test_fail labels through timeout subshells, and smoke-harness stayed portability-clean for check-portability quiet success
...
- 2026-06-01 / slice-handoff-explain-fixture-sync / ids=slice-handoff-smoke-fixture-coverage / score_delta=+1 / signal=synced the current slice-handoff helper into the smoke and execution temp repos and marked it skip-worktree so explain-mode contract checks exercise the updated code without dirtying temp worktrees
...
- 2026-06-01 / PR #210 / slice-handoff-self-modifying-helper / ids=self-modifying-helper-guard / score_delta=+4 / signal=added a preflight guard that blocks Codex prompts from editing the running slice-handoff helper itself
...
- 2026-06-03 / slice-handoff-blocker-semantics-discoverability / ids=none / score_delta=+5 / signal=added Codex final-output semantic gating before PR-body validation and submit so blocker output cannot cross the trust boundary; follow-up docs now track smaller fixture helpers, explicit blocker/output/submit summary buckets, downstream-negative assertions, trust-boundary test placement, and self-describing quiet artifacts
- 2026-06-05 / slice-handoff-blocker-semantics-discoverability / ids=slice-handoff-blocker-semantics-discoverability / score_delta=-2 / signal=child boundary failures now carry command class, artifact paths, and next-action details, and Codex blocker output stops before PR-body validation or submit when the first non-empty final line is `blocker`
...
- 2026-06-07 / slice-handoff-contract-modularity / ids=none / score_delta=0 / signal=split the slice-handoff smoke wrapper into named scenario checks, added reusable expected-text builders in the contract library, and moved pr_review_prompt_id preset lookup to repo-root .prompts for out-of-tree handoffs without changing helper behavior
- 2026-06-07 / run-tests output-contract baseline / ids=run-tests-umbrella-failure-reason-fallback / score_delta=+2 / signal=run-tests umbrella failure summaries needed a child-log/excerpt fallback so generic `failed` lines could still surface the real child reason while keeping JSON on result/status
- 2026-06-07 / codex-status recent-session list mode / ids=codex-status-recent-fixture-isolation / score_delta=+1 / signal=recent-session smoke coverage needed a separate CODEX_HOME fixture tree and explicit helper-arg references so malformed newest files and ShellCheck warnings would not obscure the recent-mode contract
