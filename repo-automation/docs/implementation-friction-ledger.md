...
| `slice-handoff-contract-opacity` | 4 | mitigated-monitoring | PR #228 | the slice-handoff smoke wrapper still needs careful first-failure capture when child-helper wiring or manifest-path propagation shifts the visible failure boundary. | keep first-failure extraction, durable failure logs, and manifest-path assertions aligned |
...
- 2026-06-06 / PR #228 / slice-handoff validator wiring repair / ids=slice-handoff-contract-opacity / score_delta=+1 / signal=repo-relative runtime-helper assertions and validation-manifest path rewrites kept the smoke contract stable while preserving the validator/preflight boundary
