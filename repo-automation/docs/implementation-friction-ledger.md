...
- 2026-06-07 / PR #233 follow-up / slice-handoff execution-submit state drift / ids=slice-handoff-contract-opacity / score_delta=0 / signal=execution-submit needed one isolated fake-artifact bundle per scenario, plus explicit harness-global references in the contract library, so blocker checks could trust the run-dir artifacts instead of shared submit fixtures
