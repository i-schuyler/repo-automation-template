...
| codex adapter | `codex-run` | mutating | local adapter around `codex exec`; tests inject a fake `codex` binary through `PATH`; optional explicit model/reasoning passthrough stays argv-driven |
...
| `--json` | valid JSON only; detailed machine output | fatal wrapper errors only; JSON helpers should document `schema`, `result`, and failure fields |
