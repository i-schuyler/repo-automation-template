...
| audit | `repo-doctor`, `run-tests`, `shellcheck-ci-parity`, `pr-body-check` | broad-local / targeted-local / instant | umbrella or broad-check helpers; `shellcheck-ci-parity` derives its helper paths from `repo-automation/helper-metadata.json`; `pr-body-check` enforces the PR body contract |
